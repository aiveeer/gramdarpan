...
export async function GET() {
  try {
    // Core counts - using only models that exist in the Prisma schema
    const [
      totalProperties,
      totalTaxMasters,
      enabledTaxMasters,
      totalTaxAssessments,
      totalDemandRegisters,
      totalTaxPayments,
      totalWards,
      totalOwners,
      totalRoads,
    ...
    const namunaStatus: { number: number; name: string; nameMr: string; status: 'available' | 'partial' | 'none'; viewId: string }[] = [
      { number: 1, name: 'Budget Estimate', nameMr: 'अर्थसंकल्प/अंदाजपत्रक', status: totalTaxAssessments > 0 ? 'available' : totalProperties > 0 ? 'partial' : 'none', viewId: 'namuna-1' },
      ...
      { number: 8, name: 'Tax Assessment Register', nameMr: 'कर आकारणी नोंदवही', status: totalTaxAssessments > 0 ? 'available' : 'partial', viewId: 'namuna-8' },
      { number: 9, name: 'Tax Demand Register', nameMr: 'कर मागणी नोंदवही', status: totalDemandRegisters > 0 ? 'available' : 'partial', viewId: 'namuna-9' },
      { number: 10, name: 'Tax & Fee Receipt', nameMr: 'कर व फी बाबत पावती', status: totalTaxPayments > 0 ? 'available' : 'partial', viewId: 'namuna-10' },
      ...
      { number: 13, name: 'Employee Register', nameMr: 'कर्मचारी नोंदवही', status: totalEmployees > 0 ? 'partial' : 'none', viewId: 'namuna-13' },
      ...
      { number: 20, name: 'Estimate Register for Works', nameMr: 'कामाच्या अंदाजाची नोंदवही', status: totalWorkEntries > 0 ? 'partial' : 'none', viewId: 'namuna-20' },
      { number: 21, name: 'Employee Bill Register', nameMr: 'कर्मचाऱ्याच्या देयकाची नोंदवही', status: totalSalaryEntries > 0 ? 'partial' : 'none', viewId: 'namuna-21' },
      { number: 22, name: 'Immovable Property Register', nameMr: 'स्थावर मालमत्ता नोंदवही', status: totalImmovableProperties > 0 ? 'available' : 'partial', viewId: 'namuna-22' },
      { number: 23, name: 'Road Register', nameMr: 'ताब्यातील रस्त्यांची नोंदवही', status: totalRoadAssets > 0 ? 'available' : 'partial', viewId: 'namuna-23' },
      { number: 24, name: 'Land Register', nameMr: 'जमिनीची नोंदवही', status: totalLandAssets > 0 ? 'available' : 'partial', viewId: 'namuna-24' },
      ...
      { number: 33, name: 'Tree Register', nameMr: 'वृक्ष नोंदवही', status: totalTreeAssets > 0 ? 'available' : 'partial', viewId: 'namuna-33' },
    ];
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        totalReceiptEntries, totalPaymentEntries, totalVoucherEntries,
        ...
        totalImmovableProperties, totalRoadAssets, totalLandAssets, totalTreeAssets,
